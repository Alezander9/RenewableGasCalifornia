# Carbon Credit Markets - Overview

This section covers the various regulatory and voluntary markets that provide financial incentives for Renewable Natural Gas (RNG) production and use.

## Key Market Mechanisms

*   **[Low Carbon Fuel Standard (LCFS) Overview](lcfs_overview.md)**: California's primary mechanism for reducing the carbon intensity of transportation fuels.
*   **[Book and Claim Accounting](book_and_claim_accounting.md)**: Detailed rules for tracking and claiming environmental attributes of RNG injected into pipelines.
*   **[RFS RIN Market](rfs_rin_market.md)**: The federal Renewable Fuel Standard and its system of Renewable Identification Numbers (RINs).
*   **[Cap and Trade](cap_and_trade.md)**: California's broader economy-wide emissions trading system.
*   **[Verification and Pathways](verification_and_pathways.md)**: The process for certifying the carbon intensity of RNG projects.
*   **[LCFS Credit Prices](lcfs_credit_prices.md)**: Historical and current trends in credit pricing.
